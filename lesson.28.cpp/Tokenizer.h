...
public:
	...
	std::string Next() {
		...
		std::string::iterator itCurr = m_itCurr;
		std::string::iterator itNext = std::find_if_not(itCurr, m_str.end(), [](char c) { return std::isspace(c) == 0; });
