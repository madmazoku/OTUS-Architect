...
TEST(TestStrategy, SetPositionByOffset) {
	IoC->Resolve<IExecuteable::Ptr>("Default.Scope.Enter")->Execute();
	try {
		size_t countGetPlayerCalls = 0;
		size_t playerIdx = 0;
		Fabric::Register(
			IoC,
			"Tank.GetPlayer",
			[&countGetPlayerCalls, &playerIdx](Fabric::Args args) {
				++countGetPlayerCalls;
				UObject::Ptr pUObject = Fabric::GetRequiredArg<UObject::Ptr>(args, 0, "invalid object");
				EXPECT_EQ(pUObject, nullptr);
				return playerIdx;
		...
		EXPECT_EQ(countGetPlayerCalls, 1);
		...
		playerIdx = 1;
		pSetPositionByOffset->SetPosition(nullptr);
		EXPECT_EQ(countGetPlayerCalls, 2);
		...
		playerIdx = 0;
		pSetPositionByOffset->SetPosition(nullptr);
		EXPECT_EQ(countGetPlayerCalls, 3);
		...
		playerIdx = 1;
		pSetPositionByOffset->SetPosition(nullptr);
		EXPECT_EQ(countGetPlayerCalls, 4);
